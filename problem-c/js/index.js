...
let opponentArray = huskyGames2016.map(extractOpponent);
...
let uwLostArray = huskyGames2016.filter(huskiesLost);
console.log(uwLostArray);

//Log out an array of opponents that UW lost to. Hint: Use the `.map()` method 
//to extract the opponent names!
let uwLostOpponetsArray = huskyGames2016.filter(huskiesLost).map(extractOpponent);
console.log(uwLostOpponetsArray);
...
/*function uwLostInfo(game) {
  console.log(game.opponent + " at " + game.home + ", " + game.opponent_score + " to " + game.home_score);
}
for (let game of uwLostArray) {
  uwLostInfo(game);
}*/
uwLostArray.forEach(function(game){
  console.log(game.opponent + " at " + game.home + ", " + game.opponent_score + " to " + game.home_score);
});
...
let fumbleGames = huskyGames2016.filter(
  function(game){
...
function mostYardsPassing(gameA, gameB) {
  if (!("passing_yards" in gameA)) {
    return gameB;
  } else if (!("passing_yards" in gameB)){
    return gameA;
  }
  if (gameA.passing_yards > gameB.passing_yards){
    return gameA;
  }
  return gameB;
}
...
let mostYardsPassingGame = huskyGames2016.reduce(mostYardsPassing, {});
console.log(mostYardsPassingGame); 
...
function makeCombinedFilter(functionA, functionB){
  let functionC = function(game) {
    return (functionA(game) && functionB(game));
  };
  return functionC;
}
...
function huskiesFumble(game){
  return (game.fumbles != 0);
}

let fumbledAndLostFilter = function(game){
  return makeCombinedFilter(huskiesLost,huskiesFumble)(game);
};
...
let uwLostFumblesArray = huskyGames2016.filter(fumbledAndLostFilter);
console.log(uwLostFumblesArray);
